...
short	botan_tim_data[16][4] = {
	...
	864,	224,	864,	252,	//  8:× 
	868,	224,	864,	253,	//  9:□ 
	872,	224,	864,	254,	// 10:Ｌ２ 
	876,	224,	864,	254,	// 11:Ｌ１ 
	880,	224,	864,	254,	// 12:Ｒ２ 
	884,	224,	864,	254,	// 13:Ｒ１ 
	888,	224,	864,	255,	// 14:ｏｒ 
	892,	224,	864,	255,	// 15:＋ 
};
char	keyconfig_num_set[4][13] = {
	...
	6, 9, 8, 7, 0, 10,  2,  2, 12, 14, 13, 15, 12,
};
...
void	set_keyconfig(WINDOW *wp) {
	...
	ppCsl = (SPRITE_OBJECT **)wp->work[3];
	for (i = 0; i < 2; i++, ppCsl++) {
		spr_xyset(*ppCsl, wp->x + 86 + 88 * i, wp->y + 8);
	}
...
void	keyconfig(WINDOW *wp) {
	...
	case 0:	// 初期設定 
		...
		// 左右カーソル 
		ppCsl = (SPRITE_OBJECT **)wp->work[3] = (SPRITE_OBJECT **)memalloc(sizeof(SPRITE_OBJECT *) * 2, (void **)&wp->work[3]);
		for (i = 0; i < 2; i++, ppCsl++) {
			*ppCsl = sprobj_make();
			spr_texset(*ppCsl, 0,0, 944, 0, 928, 255);
			(*ppCsl)->r = (*ppCsl)->g = (*ppCsl)->b = 0x80;
			(*ppCsl)->pri = wp->pri - 1;
			spr_uvset(*ppCsl, 0, 944 + 6 * i, 0);
			spr_whset(*ppCsl, 8, 8);
			spr_xyset(*ppCsl, wp->x, wp->y);
		}
